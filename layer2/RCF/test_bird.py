...
import torchvision
from dataset import Ours_Dataset
...
def multi_scale_test(model, test_loader, test_list, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        #cv2.imwrite(osp.join(save_dir, '%s.jpg' % filename), ms_fuse)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-dir', help='output folder', default='../../results/layer2/RCF_train/')

    parser.add_argument('--dataset', help='root folder of dataset', default='../../dataset/CUB_200_2011/train_image_resize/')
    ...
    test_dataset  = Ours_Dataset(root=args.dataset)
    test_loader   = DataLoader(test_dataset, batch_size=1, num_workers=1, drop_last=False, shuffle=False)
    test_list = [i.split('/')[-1] for i in test_dataset.file_list]
